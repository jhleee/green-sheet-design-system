...
**25 color tokens + 컴포넌트 클래스 + 앱 레이아웃 + 다크 모드(opt-in).** 발명한 색은 없다.
...
새 토큰이 아니라 같은 25개 이름의 재정의다. 시스템 자동 전환(`prefers-color-scheme`)은 제공하지 않는다 —
"다크 = 기계"라는 신호를 페이지가 의도적으로 선택해야 하기 때문이다. 다크에서 기계 표면(`code-bg`)은
앱 배경보다 한 층 더 깊어지고, 모든 텍스트 쌍은 WCAG AA(4.5:1) 실측 통과 값이다.
값 목록은 [`foundations/colors.html`](foundations/colors.html)의 "다크 모드" 참조.
SVG 차트처럼 리터럴 hex를 쓴 곳은 자동 전환되지 않는다 — 다크 지원 차트는 CSS 변수/currentColor를 쓸 것.

### 웹폰트 — 선택적

`theme.css`는 폰트를 로드하지 않는다(시스템 폴백으로 동작). 실제 서체가 필요하면 하나 더 import 한다:

```css
@import "green-sheet-design-system/fonts.css";  /* Pretendard dynamic-subset + JetBrains Mono, font-display: swap */
```

오프라인·사내망 환경은 `fonts.css`를 셀프호스팅 경로로 교체한다.
...
| | |
|---|---|
| [`foundations/colors.html`](foundations/colors.html) | 25색 팔레트 · 의미 규칙 · 다크 모드 · 병합 이력 |
...
| **Code** | 기계가 쓴 것 — 라이트에서 배경은 `ink`와 동일값, 다크에선 한 층 더 깊다 | `code-bg` `code-fg` `code-dim` |
...
| 레이아웃 | `.app-shell` `.app-top` `.app-brand` `.app-body` `.app-side` `.app-main` `.page-head` `.split` 계열 · 모바일(≤900px) 사이드바 드로어 `.app-menu`+`.app-menu-btn` |

v0.1 별칭(`.xl`→`.sheet`, `.ft`→`.tbl`, `.qbtn`→`.btn-pill`, `.excl`/`.sub2`→`.row-excl`/`.row-accent`)은 유지된다.

## 설계 결정 노트

- **다크 모드는 opt-in만.** `prefers-color-scheme` 자동 전환은 의도적으로 제공하지 않는다 — 페이지가 선택해야 한다.
- **모바일 드로어는 CSS-only.** `:has()` 기반 체크박스 토글이라 JS가 없다 (`:has()` 미지원 구형 브라우저에서는 사이드바가 숨김 상태로 동작).
- **v0.2 접근성 수정:** `code-dim` #6B7F8C→#8397A5 (3.65→5.03:1), `red` #BE5B4E→#B04C3E (흰 텍스트 4.38→5.32:1).
